...
from apps.db.models import Conversation, User, engine
...
class ConversationControl(BasePage):
    ...
    def on_building_ui(self):
        with gr.Row():
            title_text = "Conversations" if not VP_DEMO_MODE else "Kotaemon Papers"
        ...
        with gr.Row() as self._new_delete:
            ...
            self.cb_is_public = gr.Checkbox(
                ...
                container=False,
                visible=not VP_DEMO_MODE and not VP_SSO_ENABLED,
            )
    ...
    def load_chat_history(self, user_id):
        ...
        with Session(engine) as session:
            ...
            if result is not None:
                if settings.VP_USER_CAN_SEE_PUBLIC:
                    can_see_public = (
                        result.username == settings.VP_USER_CAN_SEE_PUBLIC
                    )
                else:
                    can_see_public = True
