...
from apps.utils.hf_papers import get_recommended_papers

from apps.reasoning.prompt_optimization.mindmap import MINDMAP_HTML_EXPORT_TEMPLATE
...
VP_WEB_SEARCH_BACKEND = getattr(settings, "VP_WEB_SEARCH_BACKEND", None)
WebSearch = None
if VP_WEB_SEARCH_BACKEND:
    try:
        WebSearch = import_dotted_string(VP_WEB_SEARCH_BACKEND, safe=False)
    except (ImportError, AttributeError) as e:
        print(f"Error importing {VP_WEB_SEARCH_BACKEND}: {e}")

REASONING_LIMITS = 2 if VP_DEMO_MODE else 10
...
class ChatPage(BasePage):
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1, elem_id="conv-settings-panel") as self.conv_column:
                ...
                for index_id, index in enumerate(self._app.index_manager.indices):
                    ...
                    if VP_DEMO_MODE and is_first_index:
                ...
                self.chat_suggestion = ChatSuggestion(self._app)  # 聊天建议

                if len(self._app.index_manager.indices) > 0:
                    quick_upload_label = (
                        "Quick Upload" if not VP_DEMO_MODE else "Or input new paper URL"
                    )
                    # 文件上传部分
                    with gr.Accordion(label=quick_upload_label) as _:
                        ...
                        self.quick_urls = gr.Textbox(
                            ...
                            show_label=False,
                            elem_id=(
                                "quick-url" if not VP_DEMO_MODE else "quick-url-demo"
                            ),
                        )
    ...
    def on_register_events(self):
        ...
        if VP_DEMO_MODE and len(self._indices_input) > 0:
