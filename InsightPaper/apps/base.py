...
import gradio as gr
import pluggy
from apps import extension_protocol
from apps.components import reasonings
from apps.assets import PDFJS_PREBUILT_DIR, KotaemonTheme
from apps.exceptions import HookAlreadyDeclared, HookNotDeclared
from apps.index import IndexManager
from apps.settings import BaseSettingGroup, SettingGroup, SettingReasoningGroup
import settings
...
class BaseApp:
    ...
    def __init__(self):
        self.dev_mode = getattr(settings, "VP_MODE", "") == "dev"
        self.app_name = getattr(settings, "VP_APP_NAME", "view")
        self.app_version = getattr(settings, "VP_APP_VERSION", "")
        self.f_user_management = getattr(settings, "VP_FEATURE_USER_MANAGEMENT", False)
