...
from src.prompts.toolcall import SYSTEM_PROMPT, NEXT_STEP_PROMPT
from src.schema import TOOL_CHOICE_TYPE, AgentState, Message, ToolCall, ToolChoice
from src.tools import CreateChatCompletion, Terminate, ToolCollection, ArxivSearch, StrReplaceEditor
...
class ToolcallAgent(ReActAgent):
    ...
    available_tools: ToolCollection = ToolCollection(
        CreateChatCompletion(), ArxivSearch(), Terminate(), StrReplaceEditor()
    )
    ...
    async def act(self) -> str:
        ...
        for command in self.tool_calls:
            # Reset base63_image for each tool call
    ...
    async def execute_tool(self, command: ToolCall) -> str:
        ...
        try:
            ...
            if hasattr(result, "base64_image") and result.base64_image:
                # Store the base64_image for later use in tool_massage
                self._current_base64_image = result.base64_image
            
            # Format result for display (standard case)
            observation = (
                f"Observed output of cmd `{name}` execued: \n{str(result)}"
                if result
