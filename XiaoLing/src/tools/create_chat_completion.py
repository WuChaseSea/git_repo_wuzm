...
class CreateChatCompletion(BaseTool):
    ...
    description: str = (
        "Create a structured completion with specified output formatting."
    )

    # Typing maping for JAON schema
    type_mapping: dict = {
        str: "string",
        int: "integer",
        float: "number",
    ...
    required: List[str] = Field(default_factory=lambda: ["response"])  # 这里需要注意含义

    def __init__(self, response_type: Optional[Type] = str):
        """Initialize with a specified response type."""
        super().__init__()
    ...
    def _build_parameters(self) -> dict:
        ...
        if isinstance(self.response_type, type) and issubclass(self.response_type, BaseModel):
            schema = self.response_type.model_json_schema()  # 这里还需要后面debug一下
            return {
                "type": "object",
