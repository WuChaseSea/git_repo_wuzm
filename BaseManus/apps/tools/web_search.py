...
from apps.tools.search import SearchItem, WebSearchEngine, BaiduSearchEngine, BingSearchEngine
...
class WebContentFetcher:
    ...
    async def fetch_content(url: str, timeout: int=10) -> Optional[str]:
        ...
        try:
            ...
            logger.warning(f"Error fetch content from {url}: {e}")
            return None
...
class WebSearch(BaseTool):
    ...
    async def _try_all_engines(
        ...
        for engine_name in engine_order:
            ...
            if not search_items:
                logger.warning(f"{engine_name} searched None...")
                continue
            if failed_engines:
                logger.info(
